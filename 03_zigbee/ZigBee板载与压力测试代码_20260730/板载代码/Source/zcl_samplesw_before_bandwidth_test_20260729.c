...
#define SENSOR_SAMPLE_PERIOD    2000
...
uint16 zclSampleSw_event_loop( uint8 task_id, uint16 events )
{
  ...
  // �����������ϱ��¼�
    ...
    if ( events & SENSOR_REPORT_EVT )
    {
      ...
      static uint8 appSeq = 0;
      uint8 sensorData[20];
      afAddrType_t destAddr;
      afStatus_t txStatus;

      dht = envDht11Read();
      pressureAdc = HalAdcRead(HAL_ADC_CHANNEL_0, HAL_ADC_RESOLUTION_12);
      lightAdc = HalAdcRead(HAL_ADC_CHANNEL_7, HAL_ADC_RESOLUTION_12);
      pressureMv = (uint16)(((uint32)pressureAdc * SENSOR_ADC_REF_MV) /
                            SENSOR_ADC_POSITIVE_FS);
      lightMv = (uint16)(((uint32)lightAdc * SENSOR_ADC_REF_MV) /
                         SENSOR_ADC_POSITIVE_FS);

      if (dht.ok)
        sprintf(lcdBuf, "T:%dC H:%d%%", dht.temperature, dht.humidity);
      else
        sprintf(lcdBuf, "DHT11:ERROR");
      HalLcdWriteString(lcdBuf, HAL_LCD_LINE_1);
      sprintf(lcdBuf, "LIGHT:%d", lightAdc);
      HalLcdWriteString(lcdBuf, HAL_LCD_LINE_2);
      sprintf(lcdBuf, "WATER:%dmV", pressureMv);
      HalLcdWriteString(lcdBuf, HAL_LCD_LINE_3);

      printf("T=%u,H=%u,DHT=%u,LIGHT=%u/%umV,WATER=%u/%umV\r\n",
             dht.temperature, dht.humidity, dht.ok,
             lightAdc, lightMv, pressureAdc, pressureMv);
      ...
      // V3 environment payload: pressure, light, DHT11 and status.
      sensorData[5]  = 0x03;
      sensorData[6]  = appSeq++;
      sensorData[7]  = 0x02;
      sensorData[8]  = LO_UINT16(pressureAdc);
      sensorData[9]  = HI_UINT16(pressureAdc);
      sensorData[10] = LO_UINT16(pressureMv);
      sensorData[11] = HI_UINT16(pressureMv);
      sensorData[12] = LO_UINT16(lightAdc);
      sensorData[13] = HI_UINT16(lightAdc);
      sensorData[14] = LO_UINT16(lightMv);
      sensorData[15] = HI_UINT16(lightMv);
      sensorData[16] = dht.temperature;
      sensorData[17] = dht.humidity;
      sensorData[18] = dht.ok;
      sensorData[19] = dht.ok ? 0x00 : 0x01;
...
{ 
  if(keys & HAL_KEY_SW_6)
  {
    ...
    // ��ʾ��ʾ
    HalLcdWriteString("Resetting...", HAL_LCD_LINE_1);
    HalLcdWriteString("Please wait...", HAL_LCD_LINE_2);
